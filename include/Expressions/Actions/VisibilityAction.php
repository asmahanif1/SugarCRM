        ...
        SUGAR.util.extend(SUGAR.forms.SetVisibilityAction, SUGAR.forms.AbstractAction, {
            ...
            sidecarExec : function(context, target, hide) {
                ...
                if (field && _.isUndefined(field.wasRequired)) {
                    //add needDependenciesCheck property into context to check changes in field
                    //by required dependency formula
                    // check - do we need to check
                    // required - should use this payload if check is true 
                    context.needDependenciesCheck = { check: false, required: false };
                    field.wasRequired = field.def.required;
                }

                let isRequired = (!_.isUndefined(context.needDependenciesCheck) &&
                    context.needDependenciesCheck.check === true);
                ...
                if (hide) {
                    ...
                    if (field.wasRequired === true) {
                        let wasRequired = (isRequired) ? context.needDependenciesCheck.required : true;
                        context.setFieldRequired(this.target, wasRequired);
                    }
                }
                if (row) {
